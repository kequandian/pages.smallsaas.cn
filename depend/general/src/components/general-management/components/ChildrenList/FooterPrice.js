...
import InputText from '../GeneralTable/InputText';
...
export default class FooterPrice extends Component{
  ...
  render(){
    const { data, config, modelState, dataPool } = this.props;
    ...
    return <Fragment>
        <div className="priceBox">
          ...
          <div><Checkbox checked={ edit } onChange={ this.handleEdit } /> 实际{ config.totalPriceText }：
            <span><InputText type="number" step={ 0.01 } value={ modelState.currentItem[config.totalPriceField] || this.totalPrice } disabled={ !edit } onChange={ this.handlePriceChange.bind(this,'totalPrice') } /></span>
          </div>
        </div>
    </Fragment>
  }
