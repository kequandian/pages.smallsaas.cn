...
// 默认 /api/cms/term/config
let API = '/api/cms/term/config';
...
export default function init({
  routePath, menu, headerMenu, types, API
}) {
  console.log("term config.init:", routePath, menu, headerMenu, types);

  API && setApi(API);
  
  if (types && Array.isArray(types)) {
    configTypes.push(...types);
  }
  ...
  console.log("term config res = ", res);
  return res;
}
